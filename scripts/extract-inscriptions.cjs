...
for (let i = startIndex + startPattern.length; i < liverDataContent.length; i++) {
...
const arrayContent = liverDataContent.substring(startIndex + startPattern.length, endIndex);

// Parse the TypeScript array and convert to JSON
try {
  // Replace THREE.Vector3 constructors with simple objects
  let cleanArrayContent = arrayContent.replace(/new THREE\.Vector3\(([^)]+)\)/g, (match, coords) => {
  ...
  const outputData = {
    metadata: {
      ...
      description: "All 42 Etruscan inscriptions from the Piacenza Liver with translations and deity associations",
  ...
  console.log(`✅ Successfully extracted ${cleanInscriptions.length} inscriptions to ${outputPath}`);
  
  // Validate the output
  if (cleanInscriptions.length !== 42) {
    console.error(`❌ Expected 42 inscriptions, got ${cleanInscriptions.length}`);
    process.exit(1);
  }
