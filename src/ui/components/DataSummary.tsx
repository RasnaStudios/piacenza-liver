...
export function DataSummary() {
  ...
  const inscriptionsByGroup = liverInscriptions.reduce((acc, inscription) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: '8px' }}>
        <strong>{totalInscriptions} Etruscan inscriptions</strong> across {totalGroups} cosmological zones
      </div>
      
      {/* All Inscriptions by Group */}
      <div style={{ marginBottom: '12px' }}>
        <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>Inscriptions by Zone:</div>
        {Object.entries(inscriptionsByGroup).map(([group, inscriptions]) => (
          <div key={group} style={{ marginBottom: '6px', marginLeft: '8px' }}>
            <div style={{ fontWeight: 'bold' }}>{group.replace('_', ' ')} ({inscriptions.length}):</div>
            {inscriptions.map(inscription => (
              <div key={inscription.id} style={{ marginLeft: '12px', fontSize: '9px', marginBottom: '2px' }}>
                #{inscription.id}: {inscription.etruscanText} → {inscription.transcription}
      ...
      {/* All Deities */}
      <div style={{ marginBottom: '12px' }}>
        <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>All {totalDeities} Deities:</div>
        {Object.entries(liverGods).map(([id, deity]) => (
          <div key={id} style={{ marginLeft: '8px', fontSize: '9px', marginBottom: '2px' }}>
            {id}: {deity.name} ({deity.romanEquivalent})
          </div>
        ))}
      </div>
      ...
      {/* Cosmological Groups */}
      <div style={{ marginBottom: '12px' }}>
        <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>Cosmological Zones:</div>
      ...
      <div style={{ fontSize: '9px', opacity: 0.7, borderTop: '1px solid #333', paddingTop: '8px' }}>
        Created by Lorenzo Andraghetti (@andraghetti)<br/>
        Interactive 3D Archaeological Reconstruction<br/>
