...
export function Legend({ hasInteracted }: LegendProps) {
	...
	const getMobileControlsText = () => {
		return ["Touch: rotate", "Pinch: zoom", "Double-tap: reset view"];
	};
	...
	const legendStyles = isMobile
		? {
				...
				transition: isDragging
					? "none"
					: "bottom 0.3s ease-out, transform 0.2s ease-out, opacity 0.2s ease-out",
	...
	return (
		<Box style={containerStyles} onDoubleClick={() => setIsOpen((v) => !v)}>
			...
			<Box
				style={{
					...
					transform: isMobile
						...
						: "translateX(-50%)",
					opacity:
						isMobile && isOpen ? Math.max(0.6, 1 - dragY / 600) : undefined,
				}}
				onMouseLeave={() => {
			...
			>
				...
				<Box
					style={{
						...
						gridTemplateColumns: isMobile
							? undefined
							: "1fr 1px minmax(160px, max-content)",
				...
				>
					...
					{/* Right side: Controls */}
					<Box
						style={{ flexShrink: 0, minWidth: "160px", width: "max-content" }}
					>
						<Text
							style={{
						...
						{(isMobile ? getMobileControlsText() : getControlsText()).map(
							(control, index) => (
								<Text
									key={index}
									style={{
										...
										marginBottom:
											index ===
											(isMobile ? getMobileControlsText() : getControlsText())
												.length -
												1
												? 0
												: "1px",
