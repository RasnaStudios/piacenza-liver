    ...
    inscription.gods.forEach(god => {
      if (getGodId(god) === godId) {
        ...
        if (form) {
          ...
          const godIndex = inscription.gods.findIndex(g => getGodId(g) === godId)
...
export function getGodVariationInInscription(godId: string, inscriptionId: number): string | null {
  const inscription = liverInscriptions.find(insc => insc.id === inscriptionId)
...
export function getGodInscriptionData(godId: string) {
  ...
  const combinedEntries = inscriptions.map(inscription => {
    ...
    const godVariation = getGodVariationInInscription(godId, inscription.id) || 
    ...
    if (otherGods.length > 0) {
      const otherGodNames = otherGods.map(god => (liverGods as any)[getGodId(god)]?.name || getGodId(god)).join(', ')
